...
class _LINE(object):
    ...
    def build_graph(self):
        ...
        cur_seed = random.getrandbits(32)
        self.embeddings = tf.get_variable(name="embeddings"+str(time.time()), shape=[self.node_size, self.rep_size], initializer = tf.contrib.layers.xavier_initializer(uniform = False, seed=cur_seed))
        #self.embeddings = tf.Variable(shape=[self.node_size, self.rep_size], initializer = tf.contrib.layers.xavier_initializer(uniform = False, seed=cur_seed))
        self.context_embeddings = tf.get_variable(name="context_embeddings"+str(time.time()), shape=[self.node_size, self.rep_size], initializer = tf.contrib.layers.xavier_initializer(uniform = False, seed=cur_seed))
        #self.context_embeddings = tf.Variable(shape=[self.node_size, self.rep_size], initializer = tf.contrib.layers.xavier_initializer(uniform = False, seed=cur_seed))
